    ...
    if (response.kind === AuthorizationResponseKind.Authorized) {
      ...
      if (response.kind === AuthorizationResponseKind.Error) {
        ...
        this.setState({
          ...currentState,
          loading: false,
          error: new Error('A personal access token cannot be used to login to Kactus.'),
        })
    ...
    if (response.kind === AuthorizationResponseKind.Authorized) {
      ...
      switch (response.kind) {
        ...
        case AuthorizationResponseKind.PersonalAccessTokenBlocked:
          this.emitError(new Error('A personal access token cannot be used to login to Kactus.'))
          break
