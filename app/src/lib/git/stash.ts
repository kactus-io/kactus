 ...
 */
const kactusStashEntryMessageRe = /!!Kactus<(.+)>$/

type StashResult = {
  /** The stash entries created by Desktop */
  readonly kactusEntries: ReadonlyArray<IStashEntry>
...
export async function getStashes(repository: Repository): Promise<StashResult> {
  ...
  if (result.exitCode === 128) {
    return { kactusEntries: [], stashEntryCount: 0 }
  }

  const kactusStashEntries: Array<IStashEntry> = []
  ...
  return {
    kactusEntries: kactusStashEntries,
    stashEntryCount: entries.length - 1,
  }
}

/**
 * Returns the last Kactus created stash entry for the given branch
 */
export async function getLastKactusStashEntryForBranch(
  ...
  return (
    stash.kactusEntries.find(stash => stash.branchName === branchName) || null
  )
}
...
async function getStashEntryMatchingSha(repository: Repository, sha: string) {
  const stash = await getStashes(repository)
  return stash.kactusEntries.find(e => e.stashSha === sha) || null
}
 ...
 */
export async function dropKactusStashEntry(
  repository: Repository,
  stashSha: string
) {
...
function extractBranchFromMessage(message: string): string | null {
  const match = kactusStashEntryMessageRe.exec(message)
...
export async function getStashedFiles(
  ...
  const [trackedFiles, untrackedFiles] = await Promise.all([
    getChangedFilesWithinStash(repository, sketchFiles, stashSha),
    getChangedFilesWithinStash(repository, sketchFiles, `${stashSha}^3`),
  ])
 ...
 */
async function getChangedFilesWithinStash(
  repository: Repository,
  sketchFiles: ReadonlyArray<IKactusFile>,
  sha: string
) {
