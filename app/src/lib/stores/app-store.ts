...
export class AppStore extends TypedBaseStore<IAppState> {
  ...
  public async _skipInstallSketchTutorialStep(repository: Repository) {
    this.tutorialAssessor.skipInstallSketch()
    await this.updateCurrentTutorialStep(repository)
  }
  ...
  public async loadInitialState() {
    ...
    this._checkSketchLocation().catch(e =>
      log.error('Failed resolving Sketch at startup', e)
    )
  ...
  private refreshIndicatorForRepository = async (repository: Repository) => {
    ...
    if (await this.shouldBackgroundFetch(repository, lastPush)) {
      const aheadBehind =
        (await this.fetchForRepositoryIndicator(repository)) || null
  ...
  private async checkoutAndBringChanges(
    ...
    try {
      ...
      const stash = (await this.createStashEntry(repository, branch))
        ? await getLastKactusStashEntryForBranch(repository, branch)
        : null
  ...
  public _openSketchFile(
    ...
    if (repository && sha) {
      ...
      return shell.openPath(sketchStoragePath + '.sketch')
    }
    return shell.openPath(file.path + '.sketch')
  }
  ...
  public async _setAppFocusState(isFocused: boolean): Promise<void> {
    ...
    if (this.appIsFocused) {
      ...
      if (this.selectedRepository instanceof Repository) {
        ...
        if (this.currentOnboardingTutorialStep === TutorialStep.InstallSketch) {
  ...
  public async _removeRepository(
    ...
    try {
      ...
      if (repository instanceof CloningRepository) {
        ...
        await clearKactusCache(undefined, String(repository.id))
  ...
  private async withAuthenticatingUser<T>(
    ...
    if (premiumType && !repository.isTutorialRepository) {
  ...
  private async _checkSketchLocation(): Promise<void> {
    const newVersion = await getSketchVersion(this.sketchPath, true)

    if (newVersion !== this.sketchVersion) {
      this.sketchVersion = newVersion

      // Make sure we let the tutorial assesor know that we have Sketch
      // in case it's stuck waiting for it to be installed.
      if (this.currentOnboardingTutorialStep === TutorialStep.InstallSketch) {
        if (this.selectedRepository instanceof Repository) {
  ...
  public async _changeSketchLocation(sketchPath: string): Promise<void> {
    ...
    return this._checkSketchLocation()
  }

  public getSketchVersion = () => {
    return this.sketchVersion
  }
  ...
  public async _resolveConflict(
    ...
    await checkoutConflictedFile(repository, file, option)
    await this._loadStatus(repository, {
      skipParsingModifiedSketchFiles: true,
    })
  }
  ...
  private async createStashAndDropPreviousEntry(
    ...
    const entry = await getLastKactusStashEntryForBranch(repository, branch)
    const gitStore = this.gitStoreCache.get(repository)
  ...
  private async createStashEntry(repository: Repository, branch: Branch) {
    ...
    return await createKactusStashEntry(repository, branch, untrackedFiles)
  }
