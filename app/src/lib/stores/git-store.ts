...
export class GitStore extends BaseStore {
  ...
  private async undoFirstCommit(
    ...
    const status = await this.performFailableOperation<IStatusResult | null>(
      () => getStatus(repository, sketchFiles)
    )
  ...
  public async undoCommit(
    ...
    const success = await this.performFailableOperation(() =>
      commit.parentSHAs.length === 0
        ? this.undoFirstCommit(this.repository, sketchFiles)
  ...
  public async loadStashEntries(
    ...
    for (const entry of stash.kactusEntries) {
    ...
    this.loadFilesForCurrentStashEntry(sketchFiles)
  }
  ...
  private async loadFilesForCurrentStashEntry(
    ...
    const files = await getStashedFiles(
      this.repository,
      sketchFiles,
      stashEntry.stashSha
    )
