...
const skipInstallSketchKey = 'tutorial-install-sketch-skipped'
...
export class OnboardingTutorialAssessor {
  ...
  private installSketchSkipped: boolean = getBoolean(
    skipInstallSketchKey,
    false
  )
  ...
  public constructor(
    /** Method to call when we need to get the current Sketch version */
    private getSketchVersion: () => string | null | undefined
  ) {}
  ...
  public async getCurrentStep(
    ...
    if (!isTutorialRepo) {
    ...
    } else if (!(await this.isSketchInstalled())) {
      return TutorialStep.InstallSketch
      ...
      return TutorialStep.CreateNewFile
  ...
  private async isSketchInstalled(): Promise<boolean> {
    return this.installSketchSkipped || this.getSketchVersion() !== null
  }
  ...
  /** Call when the user opts to skip the install Sketch step */
  public skipInstallSketch = () => {
    this.installSketchSkipped = true
    setBoolean(skipInstallSketchKey, this.installSketchSkipped)
  }
  ...
  public onNewTutorialRepository = () => {
    this.installSketchSkipped = false
    localStorage.removeItem(skipInstallSketchKey)
