...
This is the checklist of things that it needs to support:
 ...
 - it comes with a command-line interface that can be launched by Kactus

If you think your editor satisfies all these please read on to understand how
Kactus integrates with each OS, and if you're still keen to integrate this
...
[`app/src/lib/editors/win32.ts`](https://github.com/kactus-io/kactus/blob/master/app/src/lib/editors/win32.ts).
...
wishes to uninstall. These entries are used by Kactus to identify
...
is the the key that Kactus needs to read the registry and find the installation for your editor.
...
Kactus needs enough information to validate the installation - usually
...
Now that Kactus knows the program is the one it expects, it can use the
...
Kactus will confirm this file exists on disk before launching - if it's
missing or lost it won't let you launch the external editor.

## macOS

The source for the editor integration on macOS is found in
[`app/src/lib/editors/darwin.ts`](https://github.com/kactus-io/kactus/blob/master/app/src/lib/editors/darwin.ts).
...
With that information, Kactus can resolve the executable and confirm it exists
...
[`app/src/lib/editors/linux.ts`](https://github.com/kactus-io/kactus/blob/master/app/src/lib/editors/linux.ts).
