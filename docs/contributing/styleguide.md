...
The [`Dispatcher`](https://github.com/kactus-io/kactus/blob/master/app/src/lib/dispatcher/dispatcher.ts)
is the entry point for most interactions with the application which update state,
and for most usages this work is then delegated to the [`AppStore`](https://github.com/kactus-io/kactus/blob/master/app/src/lib/dispatcher/app-store.ts).
